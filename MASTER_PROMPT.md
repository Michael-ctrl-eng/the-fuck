# ═══════════════════════════════════════════════════════════════
# MASTER PROMPT: BUILD "RAQIB AGENT" v1.0
...
Raqib Agent lets an Egyptian page owner connect Facebook/Instagram/WhatsApp
...
7. DB name drift between config.py default, docker-compose env,
   .env.example → pick ONE name everywhere (raqib_agent).
...
FB_APP_ID= FB_APP_SECRET= FB_VERIFY_TOKEN=raqib-verify-token
...
NOTIFICATION_FROM_EMAIL=noreply@raqib.app
APP_NAME=Raqib Agent APP_ENV=development APP_DEBUG=true APP_HOST=0.0.0.0
APP_PORT=8000
```
...
- db: postgres:16-alpine, POSTGRES_DB raqib_agent, init.sql volume
  (CREATE EXTENSION IF NOT EXISTS pg_trgm;), healthcheck pg_isready -d raqib_agent
...
seed.py: admin@raqib.app/test123 + tenant "Egyptian Fashion Store" + 6
